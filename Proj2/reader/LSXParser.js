...
LSXParser.prototype.getPrimitive = function(tag){
	...
	var id = this.getString(tag, "id");
	var type = this.getString(tag, "type");
	var argsArray = [];
	switch(type){
	case 'plane':
	 	argsArray.push(this.getValue(tag, "parts"));
		break;
	case 'vehicle':
		break;
	case 'patch':
		var order = this.getValue(tag, "order");

		argsArray.push(order);
		argsArray.push(this.getValue(tag, "partsU"));
		argsArray.push(this.getValue(tag, "partsV"));
		var final = [];
		var arrayP = [];
		for(var i = 0; i < tag.children.length; i++){
			var x = this.getValue(tag.children[i], "x");
			var y = this.getValue(tag.children[i], "y");
			var z = this.getValue(tag.children[i], "z");
			arrayP[i%3] = vec4.fromValues(x,y,z,1);
			if(i%3 == 2){
				final.push(arrayP);
			}
		}
		argsArray.push(final);
		break;
	case 'terrain':
		argsArray.push("scenes/" + this.getString(tag,"texture"));
		argsArray.push("scenes/" + this.getString(tag,"heightmap"));
		break;
	default:
		argsArray = this.getString(tag, "args").match(/[-+]?[0-9]*\.?[0-9]+/g);
...
LSXParser.prototype.getNode = function(tag){
	...
	while(tag.children[i].tagName != 'DESCENDANTS'){
		...
		case 'ROTATION':
			...
			break;
		case 'animationref':
			animations[j] = this.getString(tag.children[i], "id");
			j++;
			break;
		default:
			break;
