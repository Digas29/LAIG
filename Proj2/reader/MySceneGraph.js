/**
 * Loads information from LSX file
 * @class MySceneGraph
 * @constructor
 * @param {String} nome do ficheiro LSX
  * @param {CGFScene} cena
 */
	...
	for (var i = 0; i < elems[0].children.length; i++) {
		...
		switch (this.primitives[i].type) {
			...
			case "triangle":
				...
				break;
			case "plane":
				if(this.primitives[i].args.length != 1){
					return "Please check plane (id=" + this.primitives[i].id + ") args LEAVE to see if matches the prototype <LEAF id=\"ss\" type=\"plane\" parts=\"ff\" />";
				}
				break;
			case "patch":
				if(this.primitives[i].args.length != 4){
					return "Please check patch (id=" + this.primitives[i].id + ") args LEAVE to see if matches the prototype <LEAF id=\"ss\" type=\"patch\" order=\"ii\" partsU=\"ii\" partsV=\"ii\" >";
				}
				break;
			case "vehicle":
				if(this.primitives[i].args.length != 0){
					return "Please check patch (id=" + this.primitives[i].id + ") args LEAVE to see if matches the prototype <LEAF id=\"ss\" type=\"vehicle\"/>";
				}
				break;
			case "terrain":
				if(this.primitives[i].args.length != 2){
					return "Please check patch (id=" + this.primitives[i].id + ") args LEAVE to see if matches the prototype <LEAF id=\"ss\" type=\"terrain\" texture=\"ss\" heightmap=\"ss\" />";
				}
				break;
			default:
...
/**
* Getting informations from animations block
...
MySceneGraph.prototype.parseAnimations = function(rootElement) {
	...
	if(elems.length != 1){
		return "either zero or more than one 'Animation' element found.";
	}

	this.animations = [];
	for(var i=0 ;i < elems[0].children.length; i++){
		var animation = elems[0].children[i];
		var id = this.parser.getString(animation, "id");
		var type = this.parser.getString(animation, "type");
		var span = this.parser.getValue(animation, "span");
		...
		if(type == "linear"){
			...
			for(var j = 0; j < animation.children.length; j++){
				...
				var x = this.parser.getValue(controlPoint,"xx");
				var y = this.parser.getValue(controlPoint,"yy");
				var z = this.parser.getValue(controlPoint,"zz");
				controlPoints.push(vec3.fromValues(x,y,z));
			}
			this.animations[id] = new LinearAnimation(id, span, controlPoints);
		}
		else if(type == "circular"){
			var radius = this.parser.getValue(animation, "radius");
			var center = this.parser.getString(animation, "center").match(/[-+]?[0-9]*\.?[0-9]+/g);
			for(var h = 0; h < center.length; h++){
				center[h] = parseFloat(center[h]);
			}
			var sAngle = this.parser.getValue(animation, "startang") * Math.PI /180.0;
			var rAngle = this.parser.getValue(animation, "rotang") * Math.PI /180.0;
			this.animations[id] = new CircularAnimation(id, span, radius, center, sAngle, rAngle);
		}
		else
